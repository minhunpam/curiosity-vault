...
### Multiplexer (MUX)
![[CON - Multiplexer.png]]
- A MUX selects 1 input out of several possible inputs and forwards only that one to the output
- The select signal (sel) determines whether out is equal to $I_0$ or $I_1$
	- Sel = 0 -> out = $I_0$
	- Sel = 1 -> out = $I_1$
> A MUX doens't compute anything, it simply routes one of the inputs to the output
### Demultiplexer (DEMUX)
