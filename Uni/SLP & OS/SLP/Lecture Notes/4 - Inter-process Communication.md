...
## `shm_open()`
- The kernel creates metadata entry  in the virtual filesystem
...
- If the file is larger than specified size, the extra data is lost. Otherwise, the file would be extended with null bytesq

## `void* mmap(void* address, size_t length, int prot, int flags, int fd, off_t offset)`
- if `address` is NULL, then the kernel chooses the (page-aligned) address at which to create the mapping (**most common way**)
	- Otherswise, the kernel would take that AS A HINT about where to place the mapping
	- if the address has already been occupied, the kernel picks a new address that may or may not related to the hint.
